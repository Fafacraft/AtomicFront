...
function generateThirdShell(scene: THREE.Scene, electron: number, orbitalRadius: number, orbitalMat: THREE.Material, shellVisibility: Record<number, boolean>, subVisibility: Record<string, boolean>) {
  ...
  // 3s Orbital
  if (subVisibility["s"] && shellVisibility[3]) {
    if (electron >= 11) {
  ...
  if (subVisibility["p"] && shellVisibility[3]) {
    // 3p Orbitals
    if (electron >= 13) {
      if (electron >= 16) {
