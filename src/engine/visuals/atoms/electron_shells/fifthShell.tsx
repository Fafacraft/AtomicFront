...
function generateFifthShell(scene: THREE.Scene, electron: number, orbitalRadius: number, orbitalMat: THREE.Material, shellVisibility: Record<number, boolean>, subVisibility: Record<string, boolean>) {
    ...
    // 5s Orbital
    if (subVisibility["s"] && shellVisibility[5]) {
        if (electron >= 37) {
    ...
    if (subVisibility["d"] && shellVisibility[4]) {
        // 4d Orbitals (39-48)
        if (electron >= 39) {
            if (electron >= 44) {
    ...
    if (subVisibility["p"] && shellVisibility[5]) {
