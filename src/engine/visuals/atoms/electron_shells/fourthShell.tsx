...
function generateFourthShell(scene: THREE.Scene, electron: number, orbitalRadius: number, orbitalMat: THREE.Material, shellVisibility: Record<number, boolean>, subVisibility: Record<string, boolean>) {
    ...
    if (subVisibility["s"] && shellVisibility[4]) {
        // 4s Orbital
        if (electron >= 19) {
    ...
    if (subVisibility["d"] && shellVisibility[3]) {
        // 3d Orbitals (21-30)
        if (electron >= 21) {
            if (electron >= 26) {
    ...
    if (subVisibility["p"] && shellVisibility[4]) {
        // 4p Orbitals (31-38)
        if (electron >= 31) {
            if (electron >= 34) {
