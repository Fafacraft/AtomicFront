...
// Function to generate second electron shell
function generateSecondShell(scene: THREE.Scene, electron: number, orbitalRadius: number, orbitalMat: THREE.Material, shellVisibility: Record<number, boolean>, subVisibility: Record<string, boolean>) {
  ...
  // 2s Orbital
  if (subVisibility["s"] && shellVisibility[2]) {
    if (electron >= 3) {
  ...
  // 2p Orbitals
  if (subVisibility["p"] && shellVisibility[2]) {
    if (electron >= 5) {
      if (electron >= 8) {
